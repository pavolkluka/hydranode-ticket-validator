...
function handleModeChange(newMode) {
    ...
    if (window.DebugLogger) {
        window.DebugLogger.info('main', 'Mode changed', {
            previousMode,
            newMode,
    ...
    // Update data availability based on new mode
    if (newMode === 'online') {
        // In online mode, check if we have authenticated user
        if (window.HydranodeAPI && window.HydranodeAPI.isAuthenticated()) {
            // Set mock data for online mode to enable scanner
            setOnlineModeData();
        } else {
            // Clear data until user authenticates
            clearCurrentData();
        }
    } else {
        // In offline mode, keep XLS data if available
        updateDataAvailability();
    }
}

function setOnlineModeData() {
    if (!window.HydranodeAPI || !window.HydranodeAPI.isAuthenticated()) {
        return;
    }
    ...
    // Create mock data structure for online mode
    const onlineData = {
        clientId: 'API_CLIENT',
        storeName: userData ? `${userData.first_name} ${userData.last_name}` : 'API User',
        data: [
            // We'll use API for actual validation, but need structure for UI
            {
                date: new Date().toLocaleDateString(),
                time: new Date().toLocaleTimeString(),
                invoiceId: 'API_VALIDATION',
                lnurlComment: 'Online API Validation Mode'
            }
        ],
        isOnlineMode: true
    };
    ...
    if (window.DebugLogger) {
        window.DebugLogger.info('main', 'Online mode data set', {
            clientId: onlineData.clientId,
            storeName: onlineData.storeName,
...
// API Data Validation Function
function validateTicketWithAPI(ticketId) {
    // This function would be called by the scanner when in online mode
    // For now, we'll implement basic validation logic
    // In a full implementation, this would make API calls to validate tickets
    ...
    // Basic ticket ID format validation
    ...
    // For now, return valid for demonstration
    return {
        isValid: true,
        status: 'valid',
        message: 'Ticket validated via API'
    };
}
